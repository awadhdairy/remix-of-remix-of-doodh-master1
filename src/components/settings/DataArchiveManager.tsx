...
export function DataArchiveManager() {
  ...
  const cutoffDate = subYears(new Date(), parseInt(retentionYears));
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Found ${previewData.reduce((sum, p) => sum + p.count, 0).toLocaleString()} records older than ${retentionYears} year(s)`,
      });
    } catch (error: any) {
      toast({
  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Successfully deleted ${response.data.totalDeleted?.toLocaleString() || 0} old records`,
      });
    } catch (error: any) {
      setResult({
        success: false,
          ...
          {previews.length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="font-medium">Records to be Deleted</h4>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button
              ...
              disabled={archiving || pin.length !== 6}
              className="gap-2"
            >
              {archiving ? (
                <Loader2 className="h-4 w-4 animate-spin" />
